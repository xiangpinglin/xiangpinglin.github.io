---
permalink: /
title: "About"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

## Bio

I am a postdoctoral scholar at [Stanford University](https://www.stanford.edu/). My research has primarily focused on [mass spectrometry](https://en.wikipedia.org/wiki/Mass_spectrometry), [metabolomics](https://en.wikipedia.org/wiki/Metabolomics), and [Olink® Proteomics](https://olink.com/technology/what-is-pea), with applications in both [biomedical research](https://en.wikipedia.org/wiki/Medical_research) and [environmental health](https://en.wikipedia.org/wiki/Environmental_health). I am particularly interested in leveraging these technologies to investigate the molecular mechanisms underlying diseases and to assess the effects of environmental exposures on human health.

## Research

My research program centers on the **[exposome](https://en.wikipedia.org/wiki/Exposome)**—the totality of environmental exposures throughout life—and its impact on human health. I specialize in:

* **Mass Spectrometry**: Developing and validating [liquid chromatography–mass spectrometry](https://en.wikipedia.org/wiki/Liquid_chromatography%E2%80%93mass_spectrometry)-based workflows using [Orbitrap](https://en.wikipedia.org/wiki/Orbitrap) and [Time-of-flight](https://en.wikipedia.org/wiki/Time_of_flight) instruments for metabolomics, [lipidomics](https://en.wikipedia.org/wiki/Lipidomics), [exposomics](https://archive.cdc.gov/www_cdc_gov/niosh/topics/exposome/default.html), and [multi-omics](https://en.wikipedia.org/wiki/Multiomics) applications
* **Environmental Health**: Investigating the health effects of environmental exposures including [air pollution](https://www.who.int/health-topics/air-pollution#tab=tab_1), [PFAS](https://en.wikipedia.org/wiki/PFAS) (per- and polyfluoroalkyl substances), and [occupational exposures](https://www.iarc.who.int/risk-factor/occupational-exposures/)

## Education

I earned my **[Ph.D.](https://en.wikipedia.org/wiki/Doctor_of_Philosophy) in [Chemistry](https://en.wikipedia.org/wiki/Chemistry)** from [Université Sorbonne Paris Nord](https://www.univ-spn.fr/), France (2020), where my dissertation focused on ["NMR and UPLC-HRMS based Metabolomics: application in Non-alcoholic fatty liver disease and Prostate Cancer Biomarker Discovery."](https://theses.hal.science/tel-03369719/) My educational foundation includes a Master's degree with honors in [Nutritional Sciences](https://en.wikipedia.org/wiki/Nutritional_science) and a Bachelor's degree in [Cell Biology](https://en.wikipedia.org/wiki/Cell_biology) and [Physiology](https://en.wikipedia.org/wiki/Physiology) from [Université Clermont Auvergne](https://www.uca.fr/), France.

My postdoctoral training included positions at Stanford University and at the [Institute for Exposomic Research](https://icahn.mssm.edu/research/exposomic) of the Icahn School of Medicine at Mount Sinai, where I developed expertise in environmental health research and advanced analytical methodologies.

## Mentoring & Teaching

I am passionate about training the next generation of scientists and actively mentor students through multiple Stanford programs:
- [Stanford Summer Research Program (SSRP)-Amgen Scholars Program](https://biosciences.stanford.edu/pathways/ssrp-amgen-scholars-program/)
- [Building Up Developing Scientists (BioBUDS) Research Internship](https://biobuds.stanford.edu/course-content/internship)  
- [Stanford Institutes of Medicine Summer Research Program](https://med.stanford.edu/simr.html) (SIMR)
- [Genomics Research Internship Program](https://med.stanford.edu/genecamp/process.html) (GRIPS)

Through these programs, I guide students in analyzing health disparities, environmental exposures, and inflammatory bowel disease research while providing comprehensive training in R programming, statistical analysis, and scientific communication.

## Professional Service

I serve on the **[Early Career Editorial Board](https://www.mdpi.com/news/14173)** of journal *[Metabolites](https://www.mdpi.com/journal/metabolites)* (2025-Present) and am an active member of the [American Society for Mass Spectrometry](https://www.asms.org/) and Stanford Medicine Children's Health Center for IBD and Celiac Disease. I regularly serve as a peer reviewer for journals including *[Nature Communications](https://www.nature.com/ncomms/)*, *[Analytical Chemistry](https://pubs.acs.org/journal/ancham)*, and *[Analytica Chimica Acta](https://www.sciencedirect.com/journal/analytica-chimica-acta)*.

## Technical Expertise

My technical skills span:
- **Analytical Chemistry**: LC-MS method development, NMR spectroscopy, sample preparation optimization
- **Computational**: R programming, statistical analysis and modeling
- **Research Areas**: Mass spectrometry, Metabolomics, Olink® Proteomics, Environmental health

## Looking Forward

My vision is to advance **precision environmental health** through innovative analytical approaches that capture the complexity of real-world exposures and their biological effects. I am particularly interested in developing standardized methodologies for exposome research and addressing environmental health disparities in vulnerable populations.

---

*Feel free to explore my [cv](https://xiangpinglin.github.io/cv/), [research](https://xiangpinglin.github.io/publications/), and [teaching](https://xiangpinglin.github.io/teaching/) pages to learn more about my work.*
